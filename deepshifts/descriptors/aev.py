...
def _cutoff_cosine(distances, cutoff):
    return torch.where(
        distances <= cutoff,
        0.5 * torch.cos(np.pi * distances / cutoff) + 0.5,
        torch.zeros_like(distances)
    )


def get_present_species(species):
...
def default_neighborlist(species, coordinates, cutoff):
    ...
    distances = distances.masked_fill(padding_mask, np.inf)
...
class AEVComputer(torch.nn.Module):
    ...
    def __init__(self, Rcr, Rca, EtaR, ShfR, EtaA, Zeta, ShfA, ShfZ,
                 species, neighborlist_computer=default_neighborlist, **kwargs):
        ...
        self.register_buffer('EtaR', torch.tensor(EtaR).view(-1,  1))
        self.register_buffer('ShfR', torch.tensor(ShfR).view( 1, -1))
        # shape convension (..., EtaA, Zeta, ShfA, ShfZ)
        self.register_buffer('EtaA', torch.tensor(EtaA).view(-1,  1,  1,  1))
        self.register_buffer('Zeta', torch.tensor(Zeta).view( 1, -1,  1,  1))
        self.register_buffer('ShfA', torch.tensor(ShfA).view( 1,  1, -1,  1))
        self.register_buffer('ShfZ', torch.tensor(ShfZ).view( 1,  1,  1, -1))
    ...
    def forward(self, species_coordinates):
        ...
        if len(coordinates.shape) == 2:
            ...
            #print(' > unsqueezing coordinates')
        
        if not isinstance(species, torch.Tensor):
            species = self.species_as_tensor(species)
            #print(' > converting species to tensor')
        
        if not isinstance(coordinates, torch.Tensor):
            coordinates = torch.tensor(coordinates, dtype=torch.float64)
            #print(' > converting coordinates to tensor')
        
        if len(species.shape) == 1:
            species = species.repeat(len(coordinates), 1)
            #print(' > replicating species')
        #print(species.shape)
        #print(coordinates.shape)
        #--------------------------------------------------
        
        present_species = get_present_species(species)
    ...
    def species_as_tensor(self, species):
        return torch.tensor([self.map[s] for s in species], dtype=torch.long)

    def __len__(self):
        return self.aev_length()
...
if __name__ == '__main__':
    ...
    computer = AEVComputer(**config).double()
    # print(computer.state_dict())

    # constants = Constants(sys.argv[1])
    # print(constants)
    # for k,v in constants.items():
    #     print(k,v)
    
    # computer = AEVComputer(**constants)
    # print(computer)
    # for func in dir(computer):
    #     if 'length' in func and callable(getattr(computer, func)):
    #         print(func, '=', getattr(computer, func)())
    Nconfs = 4
    ...
    print('species, tensor-multiple-conformations')
    print(computer((species, torch.tensor(multiple))))
    
    print('tensor-species, tensor-multiple-conformations')
    species = computer.species_as_tensor(species)
    print(computer((species, torch.tensor(multiple))))
    
    print('tensor-multiple-species, tensor-multiple-conformations')
    print(computer((species.repeat(Nconfs,1), torch.tensor(multiple))))


    # s = constants.species_to_tensor(species)    #.unsqueeze(0)
    # x = torch.from_numpy(coords.astype(np.float32))   #.unsqueeze(0)
    # s = s.repeat(3,1)
    ...
    # mol = ase.io.read(sys.argv[2])
    # print(mol)
    # print(mol.get_chemical_symbols())
    # print(mol.get_positions())
    # print(computer((mol.get_chemical_symbols(), mol.get_positions())))
